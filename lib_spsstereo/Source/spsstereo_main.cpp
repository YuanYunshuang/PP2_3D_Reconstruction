...
int main(int argc, char* argv[]) {
	if (argc < 4) {
		std::cerr << "usage: sgmstereo left right disp" << std::endl;
		exit(1);
	}
    
    std::string leftImageFilename = argv[1];
    std::string rightImageFilename = argv[2];
	std::string dispImageFilename = argv[3];
	...
	std::string outputBaseFilename = dispImageFilename;
	...
	std::string outputDisparityImageFilename = outputBaseFilename + "_disp.png";
